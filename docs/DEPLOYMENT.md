...
| `IMPORT_SCHEDULE` | nein | Cron-Ausdruck für wöchentlichen Bulk-Import | `0 2 * * 0` |
...
| `TI_RADAR_ADMIN_KEY` | nein | Admin-Key für Import-Endpunkte (leer = kein Auth) | |
...
**Hinweis:** Die Init-Skripte (`/docker-entrypoint-initdb.d/`) werden nur beim ersten Start ausgeführt (leeres pgdata-Volume). Bei bestehenden Datenbanken müssen Schema-Änderungen und Grants manuell angewandt werden.
...
- Die Datenbank belegt ca. 363 GB mit voller Datenlage (156M Patents, gefüllte
...
Für Produktions-Server, auf denen **kein git-Checkout** des Repos gewünscht
ist. Zwei Kategorien von Dateien werden benötigt:

**Aus dem `ti-radar-db`-Image unter `/opt/restore/`** (Runtime-Artefakte,
werden entweder direkt im Container ausgeführt oder per `docker cp`
extrahiert und auf dem Host gestartet):

| Datei | Zweck |
|---|---|
| `restore_split_dump.sh` | Restore-Wrapper für partitionierte Dumps (9 Phasen, auf Host ausgeführt) |
| `restore_on_server.sh` | Alternativer Wrapper für monolithische Dumps (auf Host ausgeführt) |
...
ändern sich selten und können unabhängig vom Image gezogen werden.
...
# Optional: bei ghcr.io einloggen (nur nötig wenn Images privat sind)
# echo $GHCR_PAT | docker login ghcr.io -u kingdakilla --password-stdin

# DB-Image ziehen — enthält alle Setup-Dateien unter /opt/restore/
...
Ein temporärer, niemals gestarteter Container dient nur als File-Quelle
für die Bash-Wrapper:
...
GHCR_OWNER=kingdakilla       # für ghcr.io/${GHCR_OWNER}/ti-radar-* Image-Refs
IMAGE_TAG=v3.3.4             # oder latest
SCHEDULER_ENABLED=false      # während Restore aus, später auf true
```
...
Beim ersten Start läuft automatisch das Mock-Seeding (~50 MB Demo-Daten).
Das ist OK — Phase `[3/9]` des Restore-Skripts leert alle Tabellen wieder.

### 5. Split-Dump Restore ausführen
...
Was passiert:
- Phase `[1/9]` startet die DB (läuft schon)
...
Das Skript läuft `docker compose -f $COMPOSE_FILE up -d db` — deshalb muss
...
COMPOSE_FILE=docker-compose.yml DELETE_DUMPS_AFTER_RESTORE=1 \
    ./restore_split_dump.sh /home/ben/ti_radar_dump_2026-04-08
# Ctrl+A, D zum Detachen, "screen -r restore" zum Wiederanhängen
```
...
**Einschränkung:** Das Monitoring-Profil (`prometheus` + `grafana`) benötigt
zusätzlich die Dateien unter `deploy/infra/prometheus/prometheus.yml` und
...
# Neues DB-Image ziehen (enthält ggf. aktualisierte Skripte)
docker pull ghcr.io/kingdakilla/ti-radar-db:latest

# Neue Bash-Wrapper aus dem Image extrahieren (überschreiben die alten)
docker create --name ti-radar-extract ghcr.io/kingdakilla/ti-radar-db:latest
docker cp ti-radar-extract:/opt/restore/restore_split_dump.sh ./restore_split_dump.sh
docker rm ti-radar-extract
chmod +x restore_split_dump.sh

# Optional: auch die Compose-Vorlage aus GitHub refreshen (selten nötig)
# curl -fsSL -o docker-compose.yml \
...
Für den produktiven Datenaustausch zwischen Entwicklungs- und Zielumgebung
steht ein partitionierter Split-Dump-Workflow zur Verfügung. Er ist dem
monolithischen `pg_dump` vorzuziehen, weil einzelne Patent-Dekaden individuell
kopiert, geprüft und ggf. neu übertragen werden können.
...
├── dump.sha256                         # sha256sum für jede Datei
└── patent_schema/
...
Das Skript schreibt jede Datei zunächst nach `/tmp/dump_split/` **im
Container**, kopiert sie sofort nach Host und löscht die Container-Version
...
nutzt `cygpath -m` für die `docker cp`-Destinations, weil Git Bash POSIX-
...
```bash
# Voraussetzung: Repo geclonet, Docker Compose gestartet (db-Container läuft)
...
|---|---|---|
| `COMPOSE_FILE` | `deploy/docker-compose.yml` | Compose-File für den `docker compose up -d db` Aufruf in Phase `[1/9]`. Auf dem Server `deploy/docker-compose.server.yml` setzen. |
| `DELETE_DUMPS_AFTER_RESTORE` | `0` | Wenn `1`, wird jede `.backup`-Datei auf dem Host sofort nach ihrem erfolgreichen `pg_restore` gelöscht (pro-Datei Platzsparen). `00_schema_only.sql` und `dump.sha256` bleiben erhalten. **Destruktiv** -- nach Abbruch ist kein Retry ohne erneuten Dump-Transfer möglich. |
...
Mit `DELETE_DUMPS_AFTER_RESTORE=1` verläuft der Speicherbedarf so:
- **Peak während eines Restores**: max 2x Dateigröße (Host-Datei + Container-Kopie) + DB-Wachstum
- **Nach jedem Restore**: die Host-Datei wird frei, Peak sinkt bis zum nächsten Restore
- **Kritische Phase**: `patents_2010s.backup` (20 GB -> ~100 GB DB) und `cross_schema.backup` (37 GB -> ~65 GB DB) brauchen temporär ~140 GB bzw. ~102 GB freien Speicher

`pg_restore` Exit-Code wird streng geprüft: rc=0 (Erfolg) und rc=1 (Warnings wie "already exists") führen zum Löschen; rc>1 (fataler Fehler) bricht das Skript ab, der Host-Dump bleibt für einen Retry erhalten.

Das Skript läuft in 9 Phasen:

| Phase | Beschreibung |
|---|---|
| `[1/9]` | DB-Container starten, Ready-Check |
| `[2/9]` | Postgres Performance-Tuning für Import (`max_wal_size=10GB`, `fsync=off`, `synchronous_commit=off`, `autovacuum=off`) |
| `[3/9]` | Alle Tabellen leeren, Sequenzen zurücksetzen, `document_chunks.embedding` von vector(384) auf vector(1024) anpassen |
...
| `[6/9]` | Patent-Dekaden restaurieren (pre1980 bis 2020s) -- längste Phase |
...
| `[9/9]` | Performance-Settings zurücksetzen, Container-Restart |

Phase `[7/9]` läuft das Skript `database/sql/seed_junctions_production.sql`
aus. Es ist idempotent (alle INSERTs mit `ON CONFLICT DO NOTHING`) und kann
gefahrlos erneut ausgeführt werden, wenn das Restore unterbrochen wird. Auf
dem DB-Image ist das Skript zusätzlich unter `/opt/restore/
seed_junctions_production.sql` eingebacken, sodass Phase `[7/9]` auch ohne
Repo-Checkout funktioniert.

**Geschätzte Gesamtzeit für einen vollen Restore (156M Patents):**
- Patent-Dekaden-Restore: 2-3 Stunden
- Junction-Derivation: 4-5 Stunden (primär 2010s/2020s Decades)
- MV-Refresh: 30-60 Minuten
- **Total: ca. 7-9 Stunden**

### Automatische Junction-Ableitung nach Imports

Außerhalb von Restores sorgt der **Scheduler** (`services/import-svc/src/
scheduler.py`) dafür, dass nach jedem wöchentlichen EPO-Bulk-Import die
Junction-Derivation über `junction_deriver.derive_junctions()` läuft. Für
...
Nach dem Restore eines Split-Dumps fehlen häufig die GRANT-Statements für die
...
Dump enthalten sind**, aber Service-Rollen, die später per Migration
...
Das Skript ist idempotent und kann mehrfach ausgeführt werden. Es re-granted
USAGE auf alle Schemas und SELECT auf alle Tabellen für alle 13 Service-
...
Pydantic v2 verbietet Feldnamen mit führendem Underscore. Der Fehler wurde
in `v3.3.2` behoben (HATEOAS-Feld `_links` → `links` mit Alias, sodass
JSON-Ausgabe unverändert bleibt). Upgrade auf `v3.3.2` oder höher via:
...
export_schema` auszuführen. Die Service-Rolle `svc_export` hat aber kein
`CREATE`-Recht auf der Datenbank. Seit `v3.3.2` fängt der Service den
Fehler ab und fährt weiter (Schema existiert bereits aus `002_schema.sql`).

Die Warnung `export_schema_fehler` ist kosmetisch, solange `fix_grants.sql`
ausgeführt wurde. Caching bleibt nutzbar.
...
Service nutzte fälschlicherweise `unnest()` statt `string_to_table(...,
'; ')`. Upgrade auf `v3.3.4` oder höher.
...
`entity_status`, die tatsächliche Spalte in `entity_schema.gleif_cache`
heißt `registration_status`. Umbenannt in allen SQL-Queries und der
...
Der Actor-Type-Service fängt den Fehler graceful ab und nutzt stale Cache
als Fallback. Eine Anpassung auf den aktuellen GLEIF-API-Endpoint steht
noch aus — LEI-Anreicherung ist währenddessen optional.
...
Behoben in `v3.6.3`. Stale Import in `services/embedding-svc/src/infrastructure/__init__.py` aus einem frühen Refactor. Die Klasse hieß nie `PgVectorRepository`, sondern immer `EmbeddingRepository` + `ChunkRepository`. Upgrade auf `v3.6.3+`.
...
UC1 fällt bei ungültigem Token automatisch auf die CORDIS-Zeitreihen
zurück. Ab Version `v3.3.5` wird nur noch eine einzige Warning geloggt;
die nachfolgende 403-Kaskade (22+ Zeilen pro Analyse) läuft auf
`debug`-Level, bis der nächste Refresh erfolgreich ist.
...
1. Einloggen auf <https://www.openaire.eu/user-management> (bzw. das
   ...
   **Personal Access Token** einen neuen **Refresh-Token** erzeugen.
   Der Token ist üblicherweise 1 Jahr gültig; der aus ihm abgeleitete
...
4. Im Log prüfen, dass der Refresh funktioniert. Erwartet wird genau
   eine `openaire_token_erneuert`-Zeile mit `gültig_min`-Feld,
   *keine* `openaire_token_refresh_fehlgeschlagen`-Warnings mehr:
   ...
   Zusätzlicher Smoke-Test: einmal UC1 via Orchestrator ansprechen
   ...
   Jahreszählungen wieder aus OpenAIRE geliefert statt aus dem
   CORDIS-Fallback.
