"""Master-Definitionen für Akteurs-Zählungen (Bug CRIT-3).
...
Scope-Mapping zu Query-Tabellen (dokumentarisch):
    ACTIVE_IN_WINDOW → ``entity_schema.unified_actors`` JOIN CORDIS/EPO/Publikations-
                       Aktivität im Zeitfenster. Genutzt von UC8.
    ...
    CLASSIFIED       → ``entity_schema.unified_actors`` mit gesetzter Typ-Klassi-
                       fikation (akademisch/industriell/öffentlich). Genutzt von
                       UC11.

Plausibilität: Für gleiche Tech + gleiches Zeitfenster gilt i.d.R.
``CLUSTER_MEMBER <= ACTIVE_IN_WINDOW <= CLASSIFIED`` — da klassifiziert alle
jemals kategorisierten Organisationen zählt, aktiv nur die im Fenster, und
...
class ActorScope(Enum):
    """Kanonischer Scope für Akteurs-Zählungen."""
