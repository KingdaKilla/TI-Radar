...
from src.service import RetrievalServicer
...
GRPC_REQUEST_COUNT = Counter(
    "retrieval_grpc_requests_total",
    "Anzahl gRPC-Requests",
    ["service", "method", "status"],
)
GRPC_REQUEST_DURATION = Histogram(
    "retrieval_grpc_request_duration_seconds",
...
async def serve() -> None:
    ...
    logger.info("prometheus_metrics_gestartet", port=settings.metrics_port)
    ...
    servicer = RetrievalServicer(pool=pool, settings=settings)
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set(
                "tip.retrieval.RetrievalService",
                health_pb2.HealthCheckResponse.SERVING,
            )
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
        logger.info("health_check_registriert")

    if reflection is not None and retrieval_pb2 is not None:
        service_names = (
            retrieval_pb2.DESCRIPTOR.services_by_name["RetrievalService"].full_name,
