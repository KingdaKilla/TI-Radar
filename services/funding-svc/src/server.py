"""gRPC Server für den UC4 Funding Radar Service.

Startet den async gRPC-Server mit:
- FundingServicer (UC4 EU-Förderungs-Analyse)
...
async def serve() -> None:
    ...
    servicer = FundingServicer(pool=pool, settings=settings)
    if uc4_funding_pb2_grpc is not None:
        uc4_funding_pb2_grpc.add_FundingServiceServicer_to_server(servicer, server)
        logger.info("servicer_registriert", service="FundingService")
    else:
        logger.warning(
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set(
                "tip.uc4.FundingService",
                health_pb2.HealthCheckResponse.SERVING,
            )
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
        logger.info("health_check_registriert")

    if reflection is not None and uc4_funding_pb2 is not None:
        service_names = (
            uc4_funding_pb2.DESCRIPTOR.services_by_name["FundingService"].full_name,
