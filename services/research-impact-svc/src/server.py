"""gRPC Server für den UC7 Research Impact Service.

Startet den async gRPC-Server mit:
- ResearchImpactServicer (UC7 Analyse)
...
async def serve() -> None:
    ...
    servicer = ResearchImpactServicer(pool=pool, settings=settings)
    if uc7_research_impact_pb2_grpc is not None:
        uc7_research_impact_pb2_grpc.add_ResearchImpactServiceServicer_to_server(servicer, server)
        logger.info("servicer_registriert", service="ResearchImpactService")
    else:
        logger.warning(
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set(
                "tip.uc7.ResearchImpactService",
                health_pb2.HealthCheckResponse.SERVING,
            )
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
        logger.info("health_check_registriert")

    if reflection is not None and uc7_research_impact_pb2 is not None:
        service_names = (
            uc7_research_impact_pb2.DESCRIPTOR.services_by_name["ResearchImpactService"].full_name,
