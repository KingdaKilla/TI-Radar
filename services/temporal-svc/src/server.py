...
async def serve() -> None:
    ...
    servicer = TemporalServicer(pool=pool, settings=settings)
    if uc8_temporal_pb2_grpc is not None:
        uc8_temporal_pb2_grpc.add_TemporalServiceServicer_to_server(servicer, server)
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set("tip.uc8.TemporalService", health_pb2.HealthCheckResponse.SERVING)
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
        logger.info("health_check_registriert")

    if reflection is not None and uc8_temporal_pb2 is not None:
        service_names = (
            uc8_temporal_pb2.DESCRIPTOR.services_by_name["TemporalService"].full_name,
