"""gRPC Server für den UC11 Actor Type Service."""
...
from src.service import ActorTypeServicer
...
async def serve() -> None:
    ...
    servicer = ActorTypeServicer(pool=pool, settings=settings)
    if uc11_actor_type_pb2_grpc is not None:
        uc11_actor_type_pb2_grpc.add_ActorTypeServiceServicer_to_server(servicer, server)
        logger.info("servicer_registriert", service="ActorTypeService")
    else:
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set("tip.uc11.ActorTypeService", health_pb2.HealthCheckResponse.SERVING)
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)

    if reflection is not None and uc11_actor_type_pb2 is not None:
        service_names = (
            uc11_actor_type_pb2.DESCRIPTOR.services_by_name["ActorTypeService"].full_name,
