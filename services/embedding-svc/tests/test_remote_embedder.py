"""Tests für Remote Embedding-Provider (TEI auf dediziertem GPU-Pod)."""
...
def _make_settings(**overrides) -> Settings:
    """Erzeugt Settings mit TEI-Defaults für Tests."""
    defaults = {
...
class TestRemoteEmbedder:
    """Tests für RemoteEmbedder (TEI API auf dediziertem Pod)."""
    ...
    async def test_embed_batch_returns_vectors(self, mock_client_class):
        """embed_batch sendet korrekte Anfrage und gibt Vektoren zurück."""
        ...
        # Korrekte URL und Payload prüfen (TEI-Format, kein Auth)
    ...
    async def test_embed_batch_empty_input(self, mock_client_class):
        """Leere Eingabe gibt leere Liste zurück, kein API-Aufruf."""
        mock_client = AsyncMock()
        mock_client_class.return_value = mock_client
    ...
    async def test_sub_batching(self, mock_client_class):
        """Input größer als batch_size wird in Sub-Batches aufgeteilt."""
        mock_client = AsyncMock()
        mock_client_class.return_value = mock_client
        ...
        # Drei Responses für drei Sub-Batches (TEI gibt direkt Liste zurück)
        ...
        # Prüfen, dass die Sub-Batches korrekt aufgeteilt wurden
    ...
    async def test_tei_error_raises(self, mock_client_class):
        """Nicht-200-Response löst RuntimeError aus."""
        mock_client = AsyncMock()
        mock_client_class.return_value = mock_client
    ...
    async def test_connection_error_raises(self, mock_client_class):
        """Netzwerkfehler (Timeout, DNS etc.) löst RuntimeError aus."""
        mock_client = AsyncMock()
        mock_client_class.return_value = mock_client
