"""gRPC Server für den UC5 CPC Technology Flow Service.

Startet den async gRPC-Server mit:
- CpcFlowServicer (UC5 CPC-Co-Klassifikations-Analyse)
...
from src.service import CpcFlowServicer
...
async def serve() -> None:
    ...
    servicer = CpcFlowServicer(pool=pool, settings=settings)
    if uc5_cpc_flow_pb2_grpc is not None:
        uc5_cpc_flow_pb2_grpc.add_CpcFlowServiceServicer_to_server(servicer, server)
        logger.info("servicer_registriert", service="CpcFlowService")
    else:
        logger.warning(
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set(
                "tip.uc5.CpcFlowService",
                health_pb2.HealthCheckResponse.SERVING,
            )
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
        logger.info("health_check_registriert")

    if reflection is not None and uc5_cpc_flow_pb2 is not None:
        service_names = (
            uc5_cpc_flow_pb2.DESCRIPTOR.services_by_name["CpcFlowService"].full_name,
