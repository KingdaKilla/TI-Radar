...
from src.service import GeographicServicer
...
async def serve() -> None:
    ...
    servicer = GeographicServicer(pool=pool, settings=settings)
    if uc6_geographic_pb2_grpc is not None:
        uc6_geographic_pb2_grpc.add_GeographicServiceServicer_to_server(servicer, server)
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set(
                "tip.uc6.GeographicService",
                health_pb2.HealthCheckResponse.SERVING,
            )
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
        logger.info("health_check_registriert")

    if reflection is not None and uc6_geographic_pb2 is not None:
        service_names = (
            uc6_geographic_pb2.DESCRIPTOR.services_by_name["GeographicService"].full_name,
