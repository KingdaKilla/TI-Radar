...
from src.service import TechClusterServicer
...
async def serve() -> None:
    ...
    servicer = TechClusterServicer(pool=pool, settings=settings)
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set("tip.uc9.TechClusterService", health_pb2.HealthCheckResponse.SERVING)
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
        logger.info("health_check_registriert")

    if reflection is not None and uc9_tech_cluster_pb2 is not None:
        service_names = (
            uc9_tech_cluster_pb2.DESCRIPTOR.services_by_name["TechClusterService"].full_name,
