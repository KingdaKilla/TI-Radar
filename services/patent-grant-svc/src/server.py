"""gRPC Server für den UC12 Patent Grant Rate Service."""
...
async def serve() -> None:
    ...
    servicer = PatentGrantServicer(pool=pool, settings=settings)
    if uc12_patent_grant_pb2_grpc is not None:
        uc12_patent_grant_pb2_grpc.add_PatentGrantServiceServicer_to_server(servicer, server)
        logger.info("servicer_registriert", service="PatentGrantService")
    else:
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set("tip.uc12.PatentGrantService", health_pb2.HealthCheckResponse.SERVING)
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)

    if reflection is not None and uc12_patent_grant_pb2 is not None:
        service_names = (
            uc12_patent_grant_pb2.DESCRIPTOR.services_by_name["PatentGrantService"].full_name,
