...
async def serve() -> None:
    ...
    servicer = EuroSciVocServicer(pool=pool, settings=settings)
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set("tip.uc10.EuroSciVocService", health_pb2.HealthCheckResponse.SERVING)
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)

    if reflection is not None and uc10_euroscivoc_pb2 is not None:
        service_names = (
            uc10_euroscivoc_pb2.DESCRIPTOR.services_by_name["EuroSciVocService"].full_name,
