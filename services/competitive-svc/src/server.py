"""gRPC Server für den UC3 Competitive Intelligence Service.

Startet den async gRPC-Server mit:
- CompetitiveServicer (UC3 Wettbewerbs-Analyse)
...
from src.service import CompetitiveServicer
...
async def serve() -> None:
    ...
    servicer = CompetitiveServicer(pool=pool, settings=settings)
    if uc3_competitive_pb2_grpc is not None:
        uc3_competitive_pb2_grpc.add_CompetitiveServiceServicer_to_server(servicer, server)
        logger.info("servicer_registriert", service="CompetitiveService")
    else:
        logger.warning(
    ...
    if grpc_health is not None and health_pb2_grpc is not None:
        ...
        if health_pb2 is not None:
            health_servicer.set(
                "tip.uc3.CompetitiveService",
                health_pb2.HealthCheckResponse.SERVING,
            )
            health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
        logger.info("health_check_registriert")

    if reflection is not None and uc3_competitive_pb2 is not None:
        service_names = (
            uc3_competitive_pb2.DESCRIPTOR.services_by_name["CompetitiveService"].full_name,
