...
from src.service import LandscapeServicer
...
async def serve() -> None:
    ...
    # --- Landscape Servicer registrieren ---
    servicer = LandscapeServicer(pool=pool, settings=settings)
    if uc1_landscape_pb2_grpc is not None:
        uc1_landscape_pb2_grpc.add_LandscapeServiceServicer_to_server(servicer, server)
