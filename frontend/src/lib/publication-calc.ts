 ...
 *
 * Pure Helfer für die explizite "Pub/Projekt × Projekte ≈ Publikationen"-
 ...
 * unabhängig in Vitest geprüfft werden kann.
 *
 ...
 * von 1757 bei AI). Der UI-Label nutzte bisher den externen UC1-Projektzähler
...
/** Löst den Projektzähler für die explizite Rechnung auf:
 ...
 *  - Sonst Fallback auf den externen Header-Zähler (UC1.total_projects) —
 ...
 *  Liefert `null`, wenn die Bezugsgröße fehlt (siehe `resolveProjectsCount`).
 *  Verwendet `de-DE`-Lokalisierung für die großen Zahlen, damit das Format
